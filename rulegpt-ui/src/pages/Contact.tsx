...
export function Contact() {
  return (
    <PublicPageShell
      ...
      description="RuleGPT is a trust product. The support path needs to be visible before something goes wrong, not after."
    >
      <section className="grid gap-4 md:grid-cols-3">
        {contacts.map((contact) => (
          <article key={contact.label} className="border border-black/10 bg-white px-6 py-6">
            <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{contact.label}</p>
            <p className="mt-3 text-lg font-semibold text-[#0c111d]">{contact.value}</p>
