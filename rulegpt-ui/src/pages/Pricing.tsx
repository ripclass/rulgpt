    ...
    >
      ...
      <div className="flex items-center justify-center gap-3 mb-8">
        <button
          ...
          className={`px-4 py-2 text-xs font-bold uppercase tracking-widest rounded-sm transition ${
            interval === 'monthly'
              ? 'bg-[#00261C] dark:bg-[#B2F273] text-white'
          ...
          return (
            <article
              ...
              className={`rounded-sm p-6 bg-white dark:bg-[#121212] flex flex-col border relative ${
                plan.featured
                  ? 'border-[#00261C] dark:border-[#B2F273] shadow-md shadow-[#00261C] dark:shadow-[#B2F273]/5'
                  : 'border-neutral-200 dark:border-white/10'
              }`}
            >
              {plan.badge && (
                <span className="absolute -top-3 left-4 bg-[#00261C] dark:bg-[#B2F273] text-white text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-sm">
                  {plan.badge}
                </span>
              )}
              ...
              {monthlySavings > 0 && (
                <p className="mt-1 text-xs font-medium text-[#00261C] dark:text-[#B2F273]">Save ${monthlySavings}/year</p>
              )}
              ...
              <ul className="mt-5 space-y-2.5 flex-1">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-start gap-2 text-[13px] font-medium text-neutral-600 dark:text-neutral-400">
                    <Check className="h-3.5 w-3.5 mt-0.5 text-[#00261C] dark:text-[#B2F273] shrink-0" />
                    {f}
                  </li>
                ))}
              </ul>
              ...
              <Link
                ...
                className={`mt-6 block w-full rounded-sm px-4 py-2.5 text-center text-xs font-bold uppercase tracking-widest transition ${
                  plan.featured
                    ? 'bg-[#00261C] dark:bg-[#B2F273] text-white hover:bg-[#B2F273] hover:text-neutral-900 dark:hover:bg-[#00261C] dark:hover:text-white'
      ...
      <section className="mt-12 rounded-sm p-6 bg-white dark:bg-[#121212] border border-neutral-200 dark:border-white/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          ...
          <div className="p-4 rounded-sm border-2 border-[#00261C] dark:border-[#B2F273]">
            <p className="font-bold text-[#00261C] dark:text-[#B2F273]">tfrules Professional</p>
            <p className="text-2xl font-bold text-[#00261C] dark:text-[#B2F273] mt-2">$79/mo</p>
